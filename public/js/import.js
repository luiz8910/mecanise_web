...
function enviarArquivo(indice, barra){
    ...
    if($("#system_id").val() === "")
    {
        $("#span_part_status").css('display', 'block');

        if($("#brand_parts_id").val() === "")
            $("#span_parts_brands_status").css("display", 'block');
    ...
    data.append('system_id', document.getElementById('system_id').value);
    data.append('brand_parts_id', document.getElementById('brand_parts_id').value);
    ...
    // Caminho
    request.open('post', '/importar');
    request.send(data);
}

$(function (){
    $("#system_id").change(function (){
        $("#span_part_status").css('display', 'none');
    });

    $("#brand_parts_id").change(function (){
        $("#span_parts_brands_status").css("display", 'none');
    });
})
